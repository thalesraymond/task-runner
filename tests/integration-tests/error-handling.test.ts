...
describe("Integration: Error Handling", () => {
    ...
    it("Scenario 10: Circular dependency detection", async () => {
        ...
        const steps: TaskStep<unknown>[] = [
            ...
            { name: "B", dependencies: ["C"], run: async () => ({ status: "success" }) },
    ...
    it("Scenario 11: Missing dependency handling", async () => {
        ...
        const steps: TaskStep<unknown>[] = [
             { name: "A", dependencies: ["Ghost"], run: async () => ({ status: "success" }) }
        ];

        // Validation should catch this
        await expect(runner.execute(steps)).rejects.toThrow(/depends on missing task 'Ghost'/);
    });
});
