...
runner.run(validGraph)
    .then(() => console.log("Valid graph executed successfully."))
    .catch(error => console.error("Error executing valid graph:", error.message));

// Scenario 2: Cyclic graph (will throw an error)
runner.run(cyclicGraph)
    .then(() => console.log("Cyclic graph executed (should not happen)."))
    .catch(error => console.error("Error executing cyclic graph:", error.message));
...
runner.run(missingDepGraph)
    .then(() => console.log("Graph with missing dependency executed (should not happen)."))
    .catch(error => console.error("Error executing graph with missing dependency:", error.message));

// Scenario 4: Graph with duplicate tasks (will throw an error)
runner.run(duplicateTaskGraph)
    .then(() => console.log("Graph with duplicate tasks executed (should not happen)."))
    .catch(error => console.error("Error executing graph with duplicate tasks:", error.message));
```
